...
const mapApiArea = (area: AreaApiData) => {
  ...
  const [, streetPart, housePart] =
    address.match(/ул\s([^,]+),\sд\s(\d+)/) ?? [];

  const apartment =
    area.str_number ?? area.str_number_full?.replace(/^кв\.\s*/i, '') ?? null;
...
export const AreaStore = types
  ...
  .actions((self) => ({
    fetchAreas: flow(function* (areaIds: string[]) {
      const idsToFetch = areaIds.filter((id) => id && !self.areas.has(id));
